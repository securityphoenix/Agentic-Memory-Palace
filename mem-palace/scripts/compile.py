...
KB_SYNC_START = "<!-- KB-SYNC-START -->"
KB_SYNC_END = "<!-- KB-SYNC-END -->"


def sync_doc_index() -> None:
    """Append (or update) a KB article list at the bottom of DOC_INDEX.md.

    Only the region between <!-- KB-SYNC-START --> and <!-- KB-SYNC-END --> is
    ever touched — all existing content above it is preserved.
    """
    doc_index_path = ROOT_DIR.parent / "DOC_INDEX.md"
    if not doc_index_path.exists():
        print("  --kb-sync: DOC_INDEX.md not found, skipping")
        return
    ...
    lines = []
    lines.append(f"{KB_SYNC_START}")
    lines.append("")
    lines.append("## KB Articles (auto-synced by compile.py)")
    lines.append("")
    lines.append("| Article | Path |")
    lines.append("|---------|------|")
    for article in sorted(articles):
        rel = article.relative_to(knowledge_dir)
        lines.append(f"| {rel.stem} | `knowledge/{rel}` |")
    lines.append("")
    lines.append(f"*Last synced: {now_iso()}*")
    lines.append(f"{KB_SYNC_END}")
    sync_block = "\n".join(lines)

    original = doc_index_path.read_text(encoding="utf-8")

    if KB_SYNC_START in original and KB_SYNC_END in original:
        # Replace existing sync region
        before = original[: original.index(KB_SYNC_START)]
        after = original[original.index(KB_SYNC_END) + len(KB_SYNC_END):]
        updated = before + sync_block + after
    else:
        # Append at end
        updated = original.rstrip() + "\n\n" + sync_block + "\n"

    doc_index_path.write_text(updated, encoding="utf-8")
    print(f"  --kb-sync: updated DOC_INDEX.md with {len(articles)} articles")
...
def main():
    ...
    parser.add_argument("--kb-sync", action="store_true", help="Sync KB article list into DOC_INDEX.md")
